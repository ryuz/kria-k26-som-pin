...
def k26_som_xdc_to_csv(xdc_fname, csv_fname):
    ...
    for line in src[2:]:
        ...
        if result:
            pin = result.group(1)
            pin_dict[pin]['port'] = result.group(2)
            pin_dict[pin]['bank'] = result.group(3)
            pin_dict[pin]['vcco'] = result.group(4)
            pin_dict[pin]['type'] = result.group(5)
            continue
        ...
        if result:
            ...
            pin_dict[pin]['port'] = result.group(2)
            pin_dict[pin]['bank'] = result.group(3)
            pin_dict[pin]['type'] = result.group(4)
            continue
        ...
        if result:
            pin = result.group(1)
            pin_dict[pin]['name'] = result.group(2)
            pin_dict[pin]['bank'] = result.group(3)
            pin_dict[pin]['type'] = result.group(4)
            continue
    ...
    # pandas を使って CSV に変換
    df = pd.DataFrame.from_dict(pin_dict, orient="index", columns=["port", "bank", "type", "name", "vcco"])
    df.index.name = "pin"
    df.to_csv(csv_fname)
